...
class Detector3D(nn.Module):
    ...
    def build_networks(self, model_cfg):
        """
        Almost all detector models have the following components:
            voxel feature extractor: convert a point clouds into voxels, also determine how 
                multiple points in a voxel get mapped to one voxel feature
            region proposal network: a model network that process those voxel features
            region proposal network head: a network that generate anchors (if this is a 1 stage 
                model, these are also the predictions)
            region convolutional neural network (rcnn): for 2 stages model, refine the anchors
                generated by rpn
        This function generates these components according to the model cfg provided in yaml file
        """
